...
int ata_initialize()
{
	...
	mboot_ata_interface_type = 0;
    	ide_init();


    }else if(ata_pci.adapter == ATA_AHCI_CONTROLLER){

	mboot_ata_interface_type = 1;
        ...
        printf("Controlador IDE ou AHCI nao encontrado\n");
        while(1)hlt();
    ...
    switch(ata_pci.adapter)
    {
       ...
       if(ata[dev].dev_type == ATA_DEVICE_TYPE){
        ...
        // 8 Sectors (4KB SATA or 16KB SATAPI)
          for(i=0;i < count; i+= 64) // 128kB
          {
                data = (count <= i )? 64: (count- i);
