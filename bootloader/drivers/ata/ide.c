...
static int ide_dev_init(int p)
{
    ...
    if(data == 0){
       ...
       // new_dev->dev_total_num_sector += ata_identify_dev_buf[61];
        ...
        ata[p].lba = ATA_LBA28;
        ata[p].mts =0;// (ata_identify_dev_buf[49]&0x0100)? ATA_DMA_MODO:ATA_PIO_MODO;
        ata[p].bps = 2048; 
...
{ 
	 ...
	 //select device, lba, count
        ...
        if(ata[p].lba == ATA_LBA28 && ata[p].mts == ATA_PIO_MODO){
            ...
            ide_dma_data(p,buffer,ata[p].bps*count,IDE_DMA_BUS_READ);    
...
{ 
        ...
        if(ata[p].lba == ATA_LBA28 && ata[p].mts == ATA_PIO_MODO){
            ...
            ide_dma_data(p,buffer,ata[p].bps*count,IDE_DMA_BUS_WRITE);    
