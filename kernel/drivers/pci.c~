...
void *pci_info(void *buffer,int max_bus) 
{
	...
	syscall_puts("PCI device list (classcode and programming interface), Bx:Dx:Fx\n");
		...
		for(dev = 0; dev < MAX_DEV; dev++) {
			for(fun = 0; fun < MAX_FUN; fun++) {
				...
				if(data == -1)
					continue;
				for(i = 0;i < 256;i++) {
					if((pci_classnames[i].classcode &0xffffff) != (data >> 8 &0xffffff))
						continue;
					syscall_puts(pci_classnames[i].name);
					printf(", B%d:D%d:F%d\n",bus,dev,fun);
					break;
