...
typedef struct sata{
    ...
    uint32_t np;    // Number of port
    uint32_t npm;   // Number of port multiplier
    uint32_t type;  // SATA device type
    uint32_t bps;   // byte per sector
    uint32_t mts;   // mode transfered
    uint32_t lba;   // lba type
}__attribute__((packed)) sata_t;
sata_t sata[32];
...
}__attribute__((packed)) HBA_ACMD_T;
